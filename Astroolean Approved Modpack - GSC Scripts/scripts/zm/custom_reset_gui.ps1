...
Read-Host ($Indent + 'Press Enter to exit') | Out-Null
    ...
    Write-Host ''
    Write-Host ($indent + 'ERROR: Reset failed.') -ForegroundColor Red
    if ($_.Exception -and $_.Exception.Message) {
        Write-Host ($indent + $_.Exception.Message) -ForegroundColor Red
    } else {
        Write-Host ($indent + 'Unknown error.') -ForegroundColor Red
    }

    Write-Host ''
    Write-Host ($indent + 'Details:') -ForegroundColor DarkGray
    Write-Host ($indent + ($_ | Out-String)) -ForegroundColor DarkGray
